...
class MoviesInteractorImpl(private val repository: MoviesRepository) : MoviesInteractor {
    ...
    override fun getMoviesDetails(expression: String): Flow<Pair<MovieDetails?, String?>> {
        return repository.getMovieDetails(expression).map { result ->
            when(result) {
                is Resource.Success -> {
                    Pair(result.data, null)
                }
    ...
    override fun getMovieCast(expression: String): Flow<Pair<MovieCast?, String?>> {
        return repository.getMovieCast(expression).map { result ->
            when(result) {
                is Resource.Success -> {
                    Pair(result.data, null)
                }
